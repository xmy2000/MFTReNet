...
def build_graph(solid, curv_num_u_samples, surf_num_u_samples, surf_num_v_samples):
    ...
    dgl_graph = {"graph_edge": (src, dst), "node": torch.from_numpy(graph_face_feat),
                 "edge": torch.from_numpy(graph_edge_feat)}
    return dgl_graph


def draw_face_uvgrids(solid, graph, viewer):
    a_face_data = graph["node"][0]
...
def draw_graph_edges(solid, graph, viewer):
    src, dst = graph["graph_edge"]
    num_u = graph["node"].shape[1]
    num_v = graph["edge"].shape[2]
    bbox = solid.box()
    max_length = max(bbox.x_length(), bbox.y_length(), bbox.z_length())

    for s, d in zip(src, dst):
        src_pt = graph["node"][s, num_u // 2, num_v // 2, :3].cpu().numpy()
        dst_pt = graph["node"][d, num_u // 2, num_v // 2, :3].cpu().numpy()
...
if __name__ == "__main__":
    ...
    # solid = Solid.make_cylinder(1, 1)
    solid = solid.scale_to_unit_box()
    graph = build_graph(solid, 30, 30, 5)
