...
class Solid(Shape, VertexContainerMixin, EdgeContainerMixin, ShellContainerMixin, \
    ...
    @staticmethod
    def make_box(width, height, depth, base_point=(0, 0, 0)):
        from OCC.Core.BRepPrimAPI import BRepPrimAPI_MakeBox

        gp_base = geom_utils.to_gp_pnt(base_point)
        return Solid(
            BRepPrimAPI_MakeBox(gp_base, float(width), float(height), float(depth)).Shape()
        )
