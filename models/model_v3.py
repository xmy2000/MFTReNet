...
from torch_geometric.nn import MLP, LayerNorm, GCNConv
...
class Gate(nn.Module):
    def __init__(
        ...
        self.node_attr_encoder = nn.Linear(node_attr_dim, node_attr_emb)
        self.node_grid_encoder = UVNetSurfaceEncoder(node_grid_dim, node_grid_emb)
        self.node_emb = node_attr_emb + node_grid_emb
        self.graph_encoder = GCNConv(in_channels=self.node_emb, out_channels=graph_emb)

        self.input_size = graph_emb
    ...
    def forward(self, batch_graph):
        ...
        face_attr_feat = self.node_attr_encoder(face_attr)
        face_grid_feat = self.node_grid_encoder(face_grid)
        node_feat = torch.concat([face_attr_feat, face_grid_feat], dim=1)
        graph_emb = self.graph_encoder(node_feat, edge_index)

        select = self.gate(graph_emb)
...
class CGC(nn.Module):
    def __init__(self,
        ...
        self.gate_task1 = Gate(num_specific_experts, num_shared_experts, node_attr_dim, gate_node_attr_emb,
                               node_grid_dim, gate_node_grid_emb, gate_graph_emb)
        self.gate_task2 = Gate(num_specific_experts, num_shared_experts, node_attr_dim, gate_node_attr_emb,
                               node_grid_dim, gate_node_grid_emb, gate_graph_emb)
        self.gate_task3 = Gate(num_specific_experts, num_shared_experts, node_attr_dim, gate_node_attr_emb,
                               node_grid_dim, gate_node_grid_emb, gate_graph_emb)
